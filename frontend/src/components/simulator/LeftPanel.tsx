...
function DeckZone() {
  ...
  const { drawCard, shuffleDeck } = useGameStore.getState()
...
function GraveyardZone() {
  ...
  return (
    <div className="sim-left-zone">
      ...
      <div
        ref={setNodeRef}
        className={`sim-graveyard-area ${isOver ? 'sim-zone--over' : ''}`}
      >
        {topCard && (
          <GameCardDisplay gameCard={topCard} size="md" disableDrag={false} />
        )}
      ...
      {gravCards.length > 1 && (
        <div className="sim-graveyard-scroll">
          <AnimatePresence>
            {gravCards.slice(-8).reverse().slice(1).map((card) => (
...
function ExcludedZone() {
  ...
  return (
    <div className="sim-left-zone">
      ...
      <div
        ref={setNodeRef}
        className={`sim-excluded-area ${isOver ? 'sim-zone--over' : ''}`}
      >
        {exCards.length === 0 && <div className="sim-zone-empty">なし</div>}
        {exCards.slice(-3).reverse().map((card) => (
          <GameCardDisplay key={card.instanceId} gameCard={card} size="sm" />
        ))}
      </div>
    </div>
...
function LifeZone() {
  ...
  return (
    <div className="sim-left-zone">
      ...
      <div
        ref={setNodeRef}
        className={`sim-life-area ${isOver ? 'sim-zone--over' : ''}`}
      >
        ...
        <div className="sim-life-cards">
          {lifeCards.map((id) => {
            const c = cards[id]
            return c ? (
              <GameCardDisplay key={id} gameCard={c} size="sm" showPower={false} />
            ) : null
          })}
        </div>
      </div>
