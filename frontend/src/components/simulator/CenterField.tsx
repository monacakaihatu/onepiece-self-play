...
function LeaderArea() {
  ...
  return (
    <div
      ref={setNodeRef}
      className={`sim-leader-area ${isOver ? 'sim-zone--over' : ''}`}
    >
      ...
      {leader ? (
        <div className="sim-leader-card">
          ...
          {leader.card.life && (
            <div className="sim-leader-life">LIFE: {leader.card.life}</div>
          )}
        </div>
      ) : (
...
function StageArea() {
  ...
  return (
    <div
      ref={setNodeRef}
      className={`sim-stage-area ${isOver ? 'sim-zone--over' : ''}`}
    >
...
function PlayerField() {
  ...
  return (
    <div
      ref={setNodeRef}
      className={`sim-player-field ${isOver ? 'sim-zone--over' : ''}`}
    >
...
function OppField() {
  ...
  return (
    <div
      ref={setNodeRef}
      className={`sim-opp-field ${isOver ? 'sim-zone--over' : ''}`}
    >
      ...
      <div className="sim-field-cards">
        ...
        {oppCards.length === 0 && (
          <div className="sim-field-empty">相手フィールド</div>
        )}
      </div>
    </div>
  )
